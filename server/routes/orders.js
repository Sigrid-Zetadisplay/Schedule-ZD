 ...
 *  - q (text search in title/client/tags)
 *  - client, source
 *  - limit (default 200)
 */
router.get('/orders', async (req, res) => {
  try {
    ...
    if (from || to) {
      ...
      if (bucket === 'upcoming') filter.start = { $gt: now };
      else if (bucket === 'current') filter.$and = [{ start: { $lte: now } }, { end: { $gt: now } }];
    ...
    const rows = await Order.find(filter).sort({ start: 1 }).limit(limit).lean();
    res.json(rows);
  } catch (err) {
...
/** POST /api/orders  (manual entry, e.g., Flytoget) */
router.post('/orders', async (req, res) => {
  try {
    const { title, client, sov, start, end, allDay, notes, tags, source } = req.body;
    ...
    const doc = await Order.create({
      title,
      client: client || '',
      sov: typeof sov === 'string' ? Number(sov.replace(',', '.')) : Number(sov || 0),
      start: s, end: e,
      allDay: !!allDay,
      notes: notes || '',
